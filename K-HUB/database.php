...
function getRandomQuestions($conn, $num_questions) {
    $sql = "SELECT id, question_text, answer_options, correct_answer FROM questions_g ORDER BY RAND() LIMIT $num_questions";
...
function updateUserScore($conn, $user_id, $new_score) {
    ...
    if ($new_score > $current_high_score) {
        $sql = "UPDATE users SET gramatica_score = $new_score WHERE id = $user_id";
        ...
        header("Location: gramatica.php");
        exit();
    }
}

function getCurrentHighScore($conn, $user_id) {
    $sql = "SELECT MAX(gramatica_score) AS max_score FROM users WHERE id = $user_id";
    $result = $conn->query($sql);
...
function getQuestionById($conn, $questionId) {
    ...
    $query = "SELECT * FROM questions_g WHERE id = '$questionId' LIMIT 1";
