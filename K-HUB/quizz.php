...
function getRandomQuestions($conn, $num_questions) {
    $sql = "SELECT id, question_text, answer_options, correct_answer FROM questions_v ORDER BY RAND() LIMIT $num_questions";
...
function updateUserScore($conn, $user_id, $new_score) {
    ...
    if ($new_score > $current_high_score) {
        $sql = "UPDATE users SET vocab_score = $new_score WHERE id = $user_id";
...
function getCurrentHighScore($conn, $user_id) {
    $sql = "SELECT MAX(vocab_score) AS max_score FROM users WHERE id = $user_id";
    $result = $conn->query($sql);
...
function getQuestionById($conn, $questionId) {
    ...
    $query = "SELECT * FROM questions_v WHERE id = '$questionId' LIMIT 1";
